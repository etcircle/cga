...
def _parse_env_value(value, default):
    """Parse environment string values into booleans where appropriate."""
    if value is None:
        return default
...
def _get_config_value(key, default):
    """Read debug logging knobs directly from CGA_* environment variables."""
    return _parse_env_value(os.getenv(f"CGA_{key}"), default)
