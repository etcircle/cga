...
"""
This module provides a manager for the FalkorDB Lite database connection.
FalkorDB Lite is CGA's embedded graph database backend.
"""
...
class FalkorDBUnavailableError(RuntimeError):
    ...
    or GRAPH.QUERY not available). CGA has no fallback backend; callers should
    surface this as a clear FalkorDB startup failure.
    """
...
class FalkorDBManager:
    """
    Manages the FalkorDB Lite database connection.
    ...
    def __init__(self, config: Config, graph_name: str = "codegraph"):
        """Initializes the manager from injected CGA configuration."""
        self.config = config
        self.db_path = str(config.falkordb_db_path)
        self.socket_path = str(config.falkordb_socket_path)
    ...
    def get_driver(self):
        ...
        if platform.system() == "Windows":
            raise RuntimeError(
                "CGA uses redislite/FalkorDB, which does not support Windows.\n"
                ...
                if self._driver is None:
                    ...
                    try:
                        ...
                        try:
                            ...
                            info_logger("FalkorDB Lite connection established successfully")
    ...
    def _ensure_server_running(self):
        ...
        if os.path.exists(self.socket_path):
            try:
                ...
                test_graph = d.select_graph('__cga_health_check')
        ...
        # We assume cga is installed or in python path
        if getattr(sys, 'frozen', False):
            ...
            env['CGA_RUN_FALKOR_WORKER'] = 'true'
            cmd = [python_exe]
        else:
            # If not frozen, sys.executable should be python.
            # But on some platforms (like PIP installs), it might be the 'cga' entry point script.
            ...
            cmd = [python_exe, '-m', 'cga.db.falkor_worker']
        ...
        while time.time() - start_time < timeout:
            ...
            if self._process.poll() is not None:
                ...
                # Any non-zero exit code during startup means FalkorDB cannot run here.
    ...
    def test_connection(db_path: str = None) -> Tuple[bool, Optional[str]]:
        ...
        try:
            if sys.version_info < (3, 12):
                return False, "FalkorDB Lite is not supported on Python < 3.12."
