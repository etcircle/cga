    ...
    try:
        ...
        if getattr(sys, 'frozen', False):
            ...
            if module_path:
                ...
                # Exit with a distinct code so the parent can surface that
                # FalkorDB is unavailable in this environment.
                sys.exit(2)
        ...
        try:
            test_graph = db_instance.select_graph('__cga_health_check')
            ...
            if 'graph.query' in err_str or 'unknown command' in err_str:
                ...
                sys.exit(2)  # same exit code: parent surfaces FalkorDB unavailable
            else:
